...
### 5. (オプション) 議事録＆企画書 自動生成の設定

書き起こし完了後、自動でGoogleドキュメントを作成する場合は、**別の新しいGASプロジェクト**を作成してください。

1. 新しいGASプロジェクトを作成（名前: `商談録音アプリ - 書類生成`）
2. `minutes_generator.js` の内容をコピーして貼り付け
3. **重要**: サンプル画像（商品画像）をGoogle Driveの `voice` フォルダにアップロード
   - ファイル名: `sample_product.png`
   - これがないと企画書に画像が入りません
4. トリガー設定:
   - 関数: `processDocuments`
   - 間隔: `10分ごと` または `1時間ごと`
